...
using static NetGuardPro.Services.Wfp.WfpNative;

namespace NetGuardPro.Services.Wfp;
...
///
/// Design notes:
///  - We operate a single, persistent sublayer owned by NetGuard Pro.
...
{
    // Stable identity for NetGuard Pro's sublayer. Generated once for this app;
    ...
    private void EnsureSublayer()
    {
        var sublayer = new FWPM_SUBLAYER0
        {
            ...
            {
                name = "NetGuard Pro",
                description = "NetGuard Pro firewall sublayer"
            },
    ...
    public List<ulong> EngageLockdown()
    {
        ...
        var ids = new List<ulong>(4);
        foreach (var layer in AllAppLayers())
            ids.Add(AddGlobalBlockFilter(layer, LockdownWeight, "NetGuard Lockdown"));
        return ids;
    }
    ...
    /// <summary>
    /// Tears down NetGuard Pro's entire sublayer and every filter inside it.
